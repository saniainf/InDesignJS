if (app.documents.length != 0) {
    ...
    var myStartNumber;
    var myReversePage;
    var myOffsetTop, myOffsetBottom;
...
function myDisplayDialog() {
    var myDialog = app.dialogs.add({
        name: "Нумерация спусков Офис-лидер v2.1.2",
        canCancel: true
    });
    with (myDialog) {
        var myPrintPageWidthField, myPrintPageHeightField;
        var myStartPageField, myEndPageField;
        var myStartNumberField, myCheckBoxRev, myOffsetTopField, myOffsetBottomField;
        ...
        with (dialogColumns.add()) {
            ...
            with (borderPanels.add()) {
                with (dialogColumns.add()) {
                    staticTexts.add({ staticLabel: "Клапан на форме:", minWidth: 170 });
                }
                with (dialogColumns.add()) {
                    myOffsetTopField = integerEditboxes.add({ editValue: 18, minWidth: 60 });
                }
                with (dialogColumns.add()) {
    ...
    if (myDialog.show() == true) {
        ...
        myOffsetTop = myOffsetTopField.editValue;
...
function myCaptionOnOnePage() {
    ...
    var offsetX = (myCurrentPageWidth / 2) + (myPrintPageWidth / 2);
...
function myCaptionOnTwoPages() {
    ...
    for (iPage = myStartPage; iPage <= myEndPage; iPage++) {
        ...
        offsetX = (myCurrentPageWidth / 2) + (myPrintPageWidth / 2);
